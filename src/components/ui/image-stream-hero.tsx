...
export function ImageStreamHero({ images, cards = 9, speed = 18, axis = 55, path, children, className, style, ...props }: ImageStreamHeroProps) {
  ...
  const card = `stream-card-${id}`;
  const geometry = React.useMemo(() => ({ ...PATH, ...path }), [path]);
  const css = React.useMemo(
    () => `${keyframes(1, right, geometry)}${keyframes(-1, left, geometry)}@media(prefers-reduced-motion:reduce){.${card}{animation-play-state:paused}}`,
    [right, left, card, geometry],
  );

  return (
    <div className={cn('image-stream', className)} {...props} style={{ containerType: 'inline-size', ...style }}>
      <style>{css}</style>
      <div className="image-stream__perspective" aria-hidden="true" style={{ perspective: `${geometry.perspective}cqw`, perspectiveOrigin: `50% ${axis}%` }}>
        <div className="image-stream__rails">
          {[right, left].map((animationName) => Array.from({ length: cards }, (_, index) => {
            ...
            return (
              <div
                ...
                className={cn(card, 'image-stream__card')}
                style={{
                  left: '50%',
                  top: `${axis}%`,
              ...
              >
                {image ? <Image src={image.src} alt="" fill sizes="(max-width: 720px) 35vw, 18vw" draggable={false} /> : null}
              </div>
            );
          }))}
        </div>
